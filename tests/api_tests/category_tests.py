...
def test_get_categories_success(tc_logger):
    ...
    and each category item contains all required fields, including specific category names.
           """)

    expected_categories = ["Vegan products", "Charging station", "Bike parking"]
    ...
    category_names = [category.get('name') for category in categories if 'name' in category]

    for expected_category in expected_categories:
        assert expected_category in category_names, f"Expected category '{expected_category}' not found in the response"

    tc_logger.log_test_name("Test passed: GET /categories successfully retrieved list and verified required category names")
...
def test_create_category_success(tc_logger, get_aut_token):
    ...
    tc_logger.log_test_name("Test passed: Successfully created category and verified response.")